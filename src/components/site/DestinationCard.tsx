    ...
    <motion.div variants={fadeUp} className="group relative">
      ...
      >
        ...
        <div className="absolute inset-x-6 top-6 flex items-center justify-between">
          ...
          <span className="grid h-10 w-10 place-items-center rounded-full glass text-foreground transition-colors group-hover:bg-gradient-accent group-hover:text-primary-foreground">
            <ArrowUpRight size={16} />
          </span>
        </div>
        <div className="absolute inset-x-6 bottom-6">
          <h3 className="font-display text-3xl leading-tight text-balance">{name}</h3>
          <p className="mt-2 text-sm text-foreground/75 line-clamp-2 text-pretty">{short}</p>
        </div>
      </Link>
    </motion.div>
  );
